...
func SelectResultValue(regkey, command, ip string) string {
	...
	rows, err := SqliteDB.Query("SELECT result FROM REQUEST_INFO WHERE reqkey=? and command=? and ip =? and status=1   mngOrder by time desc limit 1  ", regkey, command, ip)
	defer rows.Close()
...
func SelectCommandValue(ip, port string) (string, string, string) {
	...
	//and time > datetime('now','localtime','-2 minutes')
	rows, err := SqliteDB.Query("SELECT command,reqkey,url FROM REQUEST_INFO WHERE  ip =? and port = ? and command in ('regist','transfer') mngOrder by time desc limit 1  ", ip, port)
	defer rows.Close()
